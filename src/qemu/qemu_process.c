...
//const char *vtpmpath="/root/vtpmfiles/nvram.qcow2";
		...
		unsigned int flags)
{
	...
	/*---------------------------------------------*/
	char vtpmpath[100];
        memset(vtpmpath,0,100*sizeof(char));
        qemuDomainGetVtpmPath(vm->def->namespaceData,vtpmpath);
        FILE *op=fopen("/root/libvirt.log","a");
        if(op!=NULL){
                fprintf(op,"===========================================\n");
                fprintf(op,"in function qemuProcessStart!\n");
                if(vtpmpath!=NULL)
	...
	if (virSecurityManagerSetAllLabel(driver->securityManager,
				vm->def, stdin_path,vtpmpath) < 0)
		goto cleanup;
/*
	if(vtpmpath[0]!='0'&&virSecurityManagerSetVtpmLabel(driver->securityManager,vm->def,vtpmpath)<0){
		goto cleanup;
		...
		unsigned int flags)
{
 ...
 /*---------------------------------------------*/
        ...
        if(op!=NULL){
                fprintf(op,"===========================================\n");
                fprintf(op,"in function qemuProcessStop!\n");
        ...
        /*---------------------------------------------*/
        /*add vtpmpath parameter*/
		virSecurityManagerRestoreAllLabel(driver->securityManager,
				vm->def,
				!!(flags & VIR_QEMU_PROCESS_STOP_MIGRATED),vtpmpath);}
