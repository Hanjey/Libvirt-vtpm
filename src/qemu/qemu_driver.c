...
int getpath(char *str,char *path){
        ...
        int i,k,j;
        for(i=0;i<len;i++)
                if(str[i]=='f'&&str[i+1]=='i'&&str[i+2]=='l'){
                        j=i+5;
                        for(k=0;k<len-i;k++){
                                path[k]=str[j+k];
                                if(str[j+k]=='2'&&str[j+k-1]=='w'&&str[j+k-2]=='o'&&str[j+k-3]=='c'&&str[j+k-4]=='q'){
                                        path[k+1]='\0';
                                        break;
                                }
                        }
...
int qemuDomainGetVtpmPath(void *nsdata,char *vtpmpath);
int qemuDomainGetVtpmPath(void *nsdata,char *vtpmpath)
{
	int i;
	char *str=NULL;
