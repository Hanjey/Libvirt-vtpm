                                    ...
                                    const char *stdin_path)
{
	...
	FILE *op=fopen("/root/libvirt.log","a");
                if(op!=NULL){
                        fprintf(op,"===========================================\n");
                        fprintf(op,"in function virSecurityStackSetSecurityAllLabel!\n");
                        fprintf(op,"===========================================\n");
                        fclose(op);
                }else{
    ...
    for (; item; item = item->next) {
        if (virSecurityManagerSetAllLabel(item->securityManager, vm, stdin_path,NULL) < 0)
            rc = -1;
    }
                                        ...
                                        bool migrated)
{
    ...
    for (; item; item = item->next) {
        if (virSecurityManagerRestoreAllLabel(item->securityManager, vm, migrated,NULL) < 0)
            rc = -1;
    }
