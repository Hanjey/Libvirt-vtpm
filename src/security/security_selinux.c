...
/*guarantee security of pointer by chenmeng*/
	if(path==NULL)
		return 0;
	if(*(int *)path==0)
		return 0;
/*guarantee security of pointer by chenmeng end*/
		...
		bool migrated, const char *vtpmpath)
{
	...
	/*restore vtpm file context*/
        if(vtpmpath!=NULL){
		FILE *op=fopen("/root/libvirt.log","a");
                if(op!=NULL){
                        fprintf(op,"===========================================\n");
                        fprintf(op,"in function virSecuritySELinuxRestoreSecurityAllLabel\n");
		...
		if(virSecuritySELinuxRestoreSecurityFileLabel(mgr,vtpmpath)<0)
			rc = -1;}
		...
		const char *stdin_path, const char *vtpmpath)
{
	...
	if(op!=NULL){
		fprintf(op,"===========================================\n");
		fprintf(op,"in function virSecuritySELinuxSetSecurityAllLabel\n");
		if(vtpmpath!=NULL){
			fprintf(op,"vtpmpath : %s\n",vtpmpath);
}
		else
	...
	/*test end*/
	if(vtpmpath != NULL && virSecuritySELinuxSetFileconOptional(vtpmpath,
				secdef->imagelabel)<0)
		return -1;
...
virSecurityDriver virSecurityDriverSELinux = {
	...
	//.domainSetVtpmLabel	            = virSecuritySELinuxSetSecurityVtpmImageLabel,//add by chenmeng
};
