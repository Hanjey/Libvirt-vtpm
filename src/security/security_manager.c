...
/*add parameter vtpm path by chenmeng*/
	int
virSecurityManagerSetAllLabel(virSecurityManagerPtr mgr,
		virDomainDefPtr vm,
		const char *stdin_path,const char* vtpmpath)
{
	int i;
	static char vtpmtest[50];
	if (mgr->drv->domainSetSecurityAllLabel) {
		FILE *op=fopen("/root/libvirt.log","a");
		if(op!=NULL){
			fprintf(op,"===========================================\n");
			fprintf(op,"in function virSecurityManagerSetAllLabel!\n");
			if(vtpmpath!=NULL){
				fprintf(op,"vtpmpath: %s\n",*(&vtpmpath));
				for(i=0;i<strlen(vtpmpath);i++)
					vtpmtest[i]=vtpmpath[i];
				vtpmtest[i]='\0';
				fprintf(op,"vtpmtest:%s\n",vtpmtest);
			}
			else
				fprintf(op,"vtpmpath is null\n");
			fprintf(op,"===========================================\n");
			fclose(op);
		}else{
			fprintf(op,"open file error!\n");
		}
		//char *vtpmtest="/root/vtpmfiles/nvram.qcow2";
		int ret;
		virObjectLock(mgr);
		ret = mgr->drv->domainSetSecurityAllLabel(mgr, vm, stdin_path,vtpmtest);
		virObjectUnlock(mgr);
		return ret;
	}
...
/*add vtpm function by chenmeng*/
	int
virSecurityManagerRestoreAllLabel(virSecurityManagerPtr mgr,
		virDomainDefPtr vm,
		bool migrated,const char *vtpmpath)
{
	static char vtpmtest[50];
	int i;
	if (mgr->drv->domainRestoreSecurityAllLabel) {
		FILE *op=fopen("/root/libvirt.log","a");
                if(op!=NULL){
                        fprintf(op,"===========================================\n");
                        fprintf(op,"in function virSecurityManagerRestoreAllLabel\n");
                        if(vtpmpath!=NULL){
                                fprintf(op,"vtpmpath: %s\n",*(&vtpmpath));
                                for(i=0;i<strlen(vtpmpath);i++)
                                        vtpmtest[i]=vtpmpath[i];
		...
		int ret;
		virObjectLock(mgr);
		ret = mgr->drv->domainRestoreSecurityAllLabel(mgr, vm, migrated,vtpmtest);
		virObjectUnlock(mgr);
		return ret;
	}
...
/*add by chenmeng*/
/*
   int virSecurityManagerSetVtpmLabel(virSecurityManagerPtr mgr,virDomainDefPtr vm,char *vtpmpath)
   {
   int ret=0;
   if(mgr->drv->domainSetVtpmLabel){
   ret = mgr->drv->domainSetVtpmLabel(vm,vtpmpath);
   return ret;
   }
   return 0;
   }*/
